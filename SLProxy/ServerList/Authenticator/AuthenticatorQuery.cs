...
namespace SLProxy.ServerList.Authenticator
{
	public static class AuthenticatorQuery
	{
		public static bool SendData(IEnumerable<string> data)
		{
			...
			try
			{
				var content = new StringContent(StringUtils.ToPostArgs(data), Encoding.UTF8, "application/x-www-form-urlencoded");
				var request = HttpRequest.Client.PostAsync("https://api.scpslgame.com/v4/authenticator.php", content).Result;
				var response = request.Content.ReadAsStringAsync().Result;

				result = (response.StartsWith("{\"") ? AuthenticatorQuery.ProcessResponse(response) : AuthenticatorQuery.ProcessLegacyResponse(response));
			}
			catch (Exception ex)
			{
		...
		public static void SendContactAddress()
		{
			try
			{
				List<string> list = new List<string>
				{
					...
					"port=" + 7777,
					"version=2",
					"address=" + StringUtils.Base64Encode(ProxyConfig.Instance.Email)
				};
				...
				var content = new StringContent(StringUtils.ToPostArgs(list), Encoding.UTF8, "application/x-www-form-urlencoded");
				var request = HttpRequest.Client.PostAsync("https://api.scpslgame.com/v4/contactaddress.php", content).Result;
			}
			catch
			{
			}

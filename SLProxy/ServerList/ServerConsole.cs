...
using SLProxy.ServerList.Authenticator;
using System;
    ...
    public class ServerConsole
    {
		...
		public static void RefreshPublicKey()
		{
			...
			while (!ServerConsole._disposing)
			{
				try
				{
					var response = HttpRequest.Client.GetAsync(string.Format("{0}v4/publickey.php?major={1}", "https://api.scpslgame.com/", 11)).Result;

					PublicKeyResponse publicKeyResponse = JsonConvert.DeserializeObject<PublicKeyResponse>(response.Content.ReadAsStringAsync().Result);
		...
		public static void RefreshPublicKeyOnce()
		{
			try
			{
				var response = HttpRequest.Client.GetAsync("https://api.scpslgame.com/v4/publickey.php").Result;
		...
		public void RefreshServerData()
		{
			...
			while (!_disposing)
			{
				...
				else
				{
					...
					foreach(var server in UdpProxy.Servers.Values)
                    {
						List<string> upd = ServerConsole.Update ? new List<string>()
						{
							"ip=" + server.ServerIp,
							...
							"pastebin=" + server.Pastebin,
							"gameVersion=" + server.GameVersion,
							"version=2",
							"update=1",
							"info=" + StringUtils.Base64Encode(server.ServerName).Replace('+', '-'),
							"privateBeta=" + server.PrivateBeta,
							"staffRA=" + false,
						...
						if (!AuthenticatorQuery.SendData(upd) && !ServerConsole._printedNotVerifiedMessage)
						{
		...
		public void RunCentralServerCommand(int serverPort, string cmd, string args)
		{
			cmd = cmd.ToLower();
			...
			try
			{
				var request = new StringContent(StringUtils.ToPostArgs(list), Encoding.UTF8, "application/x-www-form-urlencoded");

				var result = HttpRequest.Client.PostAsync("https://api.scpslgame.com/centralcommands/" + cmd + ".php", request).Result;
				Console.WriteLine("[" + cmd + "] " + result.Content.ReadAsStringAsync().Result, ConsoleColor.Gray);
			}
			catch (Exception ex)
			{
